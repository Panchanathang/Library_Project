...
{
    public partial class BookPenaltyEntry : System.Web.UI.Page
    {
        SqlCommand cmd;
        ...
        private void GetMemName(string mmid)
        {
            ...
            if (dtt.Rows.Count >= 1)
            {
                ...
                txtaddress.Text = dtt.Rows[0]["address"].ToString();
                txtCity.Text = dtt.Rows[0]["city"].ToString();
                txtprovince.Text = dtt.Rows[0]["province"].ToString();
                txtzip.Text = dtt.Rows[0]["postalcode"].ToString();
            }
            else
            {
        ...
        {
            cmd = new SqlCommand("sp_InsertPenaltyDetails", dbcon.GetCon());
            ...
            cmd.Parameters.AddWithValue("@PenaltyAmount", txtAmount.Text.Trim());
            ...
            cmd.Parameters.AddWithValue("@postalcode", txtzip.Text.Trim());
            ...
            {
                ReturnBook();
                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", "swal('Success','Books Penalty Paid and Book Return Successfully','success')", true);
            }
            else
            {
